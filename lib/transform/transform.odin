...
mat4 :: proc(t: ^Transform) -> matrix[4, 4]f32 {
	m := glsl.mat4FromQuat(glsl.quat(t.rotation))
	m[0] *= t.scale.x
	m[1] *= t.scale.y
	m[2] *= t.scale.z
	m[3].xyz = t.position
...
rotatex :: proc(t: ^Transform, angle: f32) {
	t.rotation *= quaternion(glsl.cos(angle * 0.5), glsl.sin(angle * 0.5), f32(0), f32(0))
}
rotatey :: proc(t: ^Transform, angle: f32) {
	t.rotation *= quaternion(glsl.cos(angle * 0.5), f32(0), glsl.sin(angle * 0.5), f32(0))
}
rotatez :: proc(t: ^Transform, angle: f32) {
	t.rotation *= quaternion(glsl.cos(angle * 0.5), f32(0), f32(0), glsl.sin(angle * 0.5))
}
rotate_axis :: proc(t: ^Transform, axis: [3]f32, angle: f32) {
	c, s := glsl.cos(angle / 2.0), glsl.sin(angle / 2.0)
	t.rotation *= quaternion(c, s*axis.x, s*axis.y, s*axis.z)
}
